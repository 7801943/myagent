# MyAgent — 全自研 Python Agent 框架架构设计方案（V3版）
...
| 核心驱动机 | `AgentState 显式状态机` | **[V3新增]** 取代隐式While死循环，解决断线/挂起重入时的重复扣费与逻辑混乱 |
| 状态持久化 | `SQLite StateStore` | 持久化会话历史，支持异常断连后端点续传 |
| 工具幂等性 | `Idempotency Cache` 机制 | **[V3新增]** 绑定全局唯一 Tool Call ID，拦截并抵御网络超时引发的重复执行攻击 |
| 上下文把控 | `TokenBudget` + 三层结构 | **[V3新增]** System+Summary+Recent 结构；强制 Tool 结果自动截断防止爆窗 |
| 审批与策略 | `Policy Engine` 决策层 | **[V3升级]** 废弃非黑即白，支持 ALLOW/DENY/HITL 三态控制，天然联通审批挂起流 |
| 资源强隔离 | `Budget Tree 预算池` | **[V3新增]** 强制卡死大模型无限衍生 SubAgent，支持父子 Token 预算继承削减 |
| 并发与秘钥 | `Mutex` 与 `SecretManager` | 防止重入并自动脱敏凭据日志入库 |
| 链路追踪 | `TraceID / SpanID` | **[V3新增]** 统一贯穿观测日志，打通微服务级追踪能力 |
...
│     └─ Session Mutex(会话级并发保护)                              │
├──────────────────────────────────────────────────────────────────┤
│                        Agent Core (状态机内核)                     │
│   AgentLoop(显式状态转移) │ StreamProcessor(Structured Delta解析) │
│   AgentHook(生命周期钩子) │ CompositeHook │ HookContext           │
│   SubAgentManager(带Budget控制)       │ CLIProgressHook           │
│   HITL Controller(天然切入状态机恢复)                               │
├───────────────────────────┬──────────────────────────────────────┤
│      Context Manager      │           Skill Engine                │
│  (3-Tier Sliding Window)  │  (YAML/MD skills, SkillRegistry,      │
│  TokenBudget(强防爆把控)  │   composed workflows)                 │
│  └─ StateStore(全态持久化)│                                      │
├───────────────────────────┴──────────────────────────────────────┤
│                        Tool System                                │
│  ToolRegistry │ ToolExecutor │ IdempotencyCache(ToolCall并发防刷) │
│  DocumentTools │ VectorRAGTool │ SecretManager(密钥提取剥离)      │
├──────────────────────────────┬───────────────────────────────────┤
│        Safety System         │      Document Processor            │
│  GuardChain │ PolicyEngine   │  DOCX │ XLSX │ PDF(pdfplumber)    │
│  (ALLOW/DENY/HITL/REWRITE)   │  ImageHandler(多模态 base64/URL)  │
├──────────────────────────────┴───────────────────────────────────┤
│                       Provider Layer                              │
│  ProviderRouter(failover) │ OpenAIProvider │ AnthropicProvider    │
│  UnifiedStreamEvent │ RetryPolicy(分离API与Tool)                  │
│  CapabilityDetector(多模态/工具能力检测)                          │
├──────────────────────────────────────────────────────────────────┤
│               评测与 RAG Interface（迭代引擎）                    │
│     Evaluation Engine(评测框架) │ BaseVectorStore │ RAGSkill      │
├──────────────────────────────────────────────────────────────────┤
│          Observability Layer — 日志与审计（伴随 TraceID 打通）     │
│  AuditLogger │ AuditLevel(4级粒度) │ AuditHook(异步无阻塞提取)    │
│  JSONLBackend │ SQLiteBackend │ PostgresBackend(预留)             │
│  AsyncEventQueue │ FieldMasker(脱敏拦截) │ Trace/Span ID 分配     │
└──────────────────────────────────────────────────────────────────┘
```
...
│   │   └── hitl.py              #   [V2新增] 人在回路控制（审批请求与挂起/唤醒状态管理）
│   │
...
│   │   └── state.py             #   [V2新增] StateStore (Redis/SQLite 持久化会话)
│   │
│   ├── tools/                   # ⑤ 工具系统
│   │   ├── base.py              #   BaseTool, ToolResult
│   │   ├── registry.py          #   ToolRegistry
│   │   ├── executor.py          #   ToolExecutor（带安全前置检查）
│   │   ├── secrets.py           #   [V2新增] SecretManager (凭据安全获取与下发隔离)
...
│   │
│   ├── evaluation/              # [V2新增] 评测框架层
│   │   ├── engine.py            #   EvalEngine (端到端跑批验证框架)
│   │   ├── datasets.py          #   数据集加载与回放模块
│   │   └── judge.py             #   LLMJudge打分评估实现
│   │
│   ├── interfaces/              # ⑪ 接口层
...
│   │   │   └── lock.py          #   [V2新增] SessionMutex (基于 Session ID 的并发控制锁)
│   │   └── im/                  #   IM 接口预留目录
│   │       └── base.py
│   │
...
```
[V3 显式状态机驱动 ReAct] —— 彻底抛弃危险的隐式 while True
AgentState(Enum):
  IDLE | RUNNING | WAITING_TOOL | WAITING_HITL | ERROR | FINISHED

触发 resume(session_id) / 启动
  │
  ▼
从 StateStore 反序列化取得当前状态 state 及断点。
若 state 为 RUNNING / IDLE:
  │
  ▼
ProviderRouter.stream()
  │
  ├── text_delta ──► AgentHook.on_stream(delta)
  └── tool_call_end  ──► State 转为 WAITING_TOOL，将 pending_tools 存入 StateStore
  
此时开始解析 Tool Calls：
  ▼
PolicyEngine.decide(tool)  ← [V3新增] 基于动态规则表决策
  ├─ DENY: 报错拦截
  ├─ REQUIRE_HITL / 标记审批:
  │    │
  │    ▼ 将状态设为 WAITING_HITL，记录当前调用栈后终止 asyncio Task 并释放 Mutex! (等待外部调用)
  │
  └─ ALLOW:
       │
       ▼ [防击穿检查]
      IdempotencyCache.has(tool_call.id)？ ← [V3新增] 如果是挂起重跑/网络重试导致重入的调用
       ├─ 是: 引出被缓存的执行结果，拒接二次调用。
       └─ 否:
            ▼
          SecretManager.inject()        ← 脱出屏蔽秘钥注入
          asyncio.gather(*[ToolExecutor.execute])
            ▼
          IdempotencyCache.store()      ← [V3新增] 保存防丢幂等数据
          ContextManager.add_tool_result()
          State 改回 RUNNING，开启新周期进入下一轮 Iteration。
```
...
class Budget:
    ...
    def decrease(self, tokens=0, tool_calls=0, subagents=0) -> bool:
        # 每当大模型执行推演就扣除该预算配额，任何余额为 0 时抛 BudgetExceeded 终止符。
        pass
...
class SubAgentSpec:
    ...
    model: str | None = None
    budget: Budget | None = None       # [V3新增] 所有 SubAgent 强制分配独立或继承配减的 Budget 以防崩溃滥用
...
#### 1. SessionMutex (防并发请求重入锁)

在异步处理中（特别是 WebSocket 场景下），针对同一个会话（Session ID）发出多个即时消息，可能会意外导致并行启动多个 AgentLoop，产生互相覆盖的历史对话和混乱的工具执行逻辑。在接口层接入全局 Session Mutex 防并发保护。

```python
# interfaces/websocket/lock.py
class SessionMutex:
    def __init__(self):
...
#### 2. StateStore (状态存储与端点恢复)

由于内存会丢失，需要支持通过持久化存储把 Message List 及基本会话状态进行读写，使得服务或容器重启之后，或者 WebSocket 意外断连后可以用 ID 原样恢复记忆。

```python
# context/state.py
class StateStore(ABC):
    @abstractmethod
    async def save_session(self, session_id: str, messages: list[Message], metadata: dict) -> None: ...
    @abstractmethod
    async def load_session(self, session_id: str) -> tuple[list[Message], dict]: ...

# 采用轻量级原生 SQLite 驱动。并在 Provider 产生新 token/message 落地时调用。
```

#### 3. HITL 控制机制 (审批挂起机制)

遇到一些危险工具（例如执行数据库 DML、发送重要通知系统信息等），框架不能单纯靠规则阻拦，还可以通过 HITL 等待外部二次授权（Approval）。

```python
# core/hitl.py
class HITLController:
    # 挂起动作
    async def maybe_suspend(self, tool_call: ToolCall, session_id: str) -> None:
        if tool_call.name in self.requires_approval_tools:
            # 序列化当前大循环的 state (含等待结果的 ID) 存入 StateStore
            # 抛出 ApprovalRequest UI 事件
            # asyncio.CancelledError / 终止当前 Loop
            pass
            
    # 唤醒动作：由 WebSocket / API Controller 收到了授权后触发
    async def resume(self, session_id: str, tool_id: str, action: Literal["approve", "reject", "modify"], new_args: dict=None):
        # 1. Acquire Session Lock
        # 2. Context = StateStore.load(session_id)
        # 3. 构造对应的 ToolResult 或修改对应的 Action
        # 4. 继续驱动 AgentLoop() 往下走
        pass
```

#### 4. SecretManager (统一密钥凭据管理器)

避免将 GitHub Token 或云服务凭据配置在技能的 YAML 或 prompt 里，使用统一的管理组件。不仅隔离运行时获取，还要求对返回结果同步更新给 Auditor 过滤拦截。

```python
# tools/secrets.py
class SecretManager:
    def resolve_tool_credentials(self, tool_name: str) -> dict:
        # 实时根据系统环境变量/Vault拉取实际秘钥 dict，供 Tool 执行。
        pass
        
    def register_redaction(self, masker: FieldMasker, secret_val: str) -> None:
        # 当秘钥拉取到后，通知系统的 PII Masker 屏蔽这些明文字符
        masker.add_redact_pattern(secret_val)
```

#### 5. EvalEngine (评测框架)

独立于业务的主干的“性能”评测单元，利用 LLM 作为 Judge 评审框架自身的回答能力。
通过输入一个带有预期输出定义的数据集文件（如 JSONL），并发运行全自动打分来测试基线稳定性。评估每次 Prompt、工具调整有没有带来退化。
...
V2 版本解决了功能的“有无”问题，但在生产大并发与极端崩溃场景下依然脆弱。V3 版本加入了以下 5 项防御性骨架体系：

#### 1. AgentLoop：从隐式 while 向“显式状态机驱动”跃迁
传统的 ReAct 本质是一个内存 `while` 循环，但如果在某一次 Provider HTTP 请求时触发网络异常进而超时重试，或者等待触发 HITL (人在回路)，这个内存的 While 是会被断开或重复拉起的。
这会导致已经执行过的 Tool (例如发短息) 再次被拉一遍。
**V3 方案**：构建 `AgentState`。当 LLM 输出 `tool_call` 时，先把所有 Call 快照写入 `StateStore(WAITING_TOOL)`。不管 Agent 从哪次网络崩溃恢复起来，它第一步先判定当前身处的 state 再恢复下发，从流程驱动变成事件恢复拉取模式。

#### 2. Tool 系统的幂等性与重入保护机制 (Idempotency)
引入强制机制：LLM 下发的所有 `tool_call` 必须含有唯一的 `call_id`。
并在执行时校验缓存逻辑：
```python
# execution 准备阶段
if tool_call.id in state_store.tool_call_results:
    return cached_result # 跳过工具的二次实体执行
execute_tool(...)
```
这样即便因为网络超时或集群重启重新推拉 Message 序列，“发一封邮件”或“删除一个云资源”这种不负责任的操作也不会由于架构的 Failover 重试被触发第二次。

#### 3. ContextManager：Token Budget 与“三明治控制法”
纯文本拼接历史很快会在长时间会话或爬虫类 Tool 结果爆仓模型上限。
**V3 将强制对 Context 执行三元限制**：
1. **三层排布模型**：`[System Prompt] + [Summary Memory (由旧消息被动触发摘要)] + [Recent M轮原话]` 永远保证上下文是高度富集的结构化块，不再一味追加。
2. **全局 Token Budget** 控制点。
3. **工具输出强截断**：`ToolResult(content=truncate(content, max=4000), full_blob_ref="uuid")`。无论检索网页多差劲，也绝不把超量的无用结构喂进 prompt 栈。

#### 4. Provider 的统一 “Structured Tool Call Delta”
如果任由 Provider 随意拼串返回 `tool_args_delta: str` 进框架，后续一旦 JSON 引号错位 debug 会非常窒息。
引入中间协议：
```python
class ToolCallDelta:
    name: str
    arguments_partial: dict # 在流式内部自行处理合并 JSON 串的逻辑，容错后再对 Agent 抛出成型的键值字典
```

#### 5. Safety System 的“Policy Engine (策略层)” 升级
安全护栏不能非黑即白：
升级安全网不限于 `ALLOW` 和 `DENY`，改为：
```python
class PolicyDecision(Enum):
    ALLOW = 1
    DENY = 2
    REQUIRE_HITL = 3  # 高危动态命令直接切入挂起授权态
    REWRITE = 4       # 修改传入参数并放行
```
做到自动批准低风险操作，HITL人工拦截中风险（如操作核心文件），静默阻拦极高危（逃逸 Shell）。

#### 6. 全局的 Trace_id / Span_id 穿透 (Observability 进化)
为每一个 `Session ID` 下发生的 Turn 及其子 Tool 执行、SubAgent 生成唯一链条 `trace_id`。所有 `AuditLogger` 打入日志均带入该标识符。配合 ELK 或普通数据库查询，直接串讲整个 LLM 的“树状思维过程”。
...
# [V2新增] 安全密钥与评测
hvac = {version = "^2.0", optional = true} # HashiCorp Vault
# (EvalEngine 评测框架使用纯 Python 原生 json/jsonl 解析，去除重型数据依赖)
...
- [ ] **[V2] StateStore 持久化接口**（基于本地 SQLite 实现对话原样加载与追加）
- [ ] **[V2] SessionMutex 并发锁**（防御 WebSocket 环境下的读写同传重入崩溃）
...
- [ ] **[V2] HITL Controller 机制**（引入工具执行挂起机制并抛出外部审批请求信号）
- [ ] **[V2] SecretManager 统一凭据提取**（结合 FieldMasker 脱敏机制实现密文入库拦截）
...
- [ ] 单元与集成基础测试覆盖
- [ ] **[V2] EvalEngine 独立评测框架体系**（定义基于 LLM judge 测试金标准的指令回归护栏能力）
- [ ] README + 配置文档
