# MyAgent — 全自研 Python Agent 框架架构设计方案（V2版）
...
| 状态持久化 | `SQLite/Redis StateStore` | **[V2新增]** 持久化会话历史，支持异常断连后端点续传 |
| 人在回路(HITL) | `HITL Controller` 挂起机制 | **[V2新增]** 拦截高危操作流，等待人类审批或补充参数后恢复执行 |
| 密钥安全管理 | `SecretManager` 后期注入 | **[V2新增]** 工具层面凭据与配置隔离，按需获取并在日志记录中自动脱敏 |
| 并发会话控制 | `SessionMutex` 请求互斥锁 | **[V2新增]** 防止单会话状态下的高并发产生重入和上下文混乱 |
| 自动化评测 | `EvalEngine` | **[V2新增]** LLM 辅助作为 judge 运行黄金数据集回归测试 |
...
│  DocumentTools │ VectorRAGTool │ SecretManager(密钥下发机制)      │
├──────────────────────────────┬───────────────────────────────────┤
...
用户输入
  │
  ▼
SessionMutex.acquire(session_id)   ←── [V2新增] 拦截保护，防止同一 session 并发重入
  │
  ▼
ContextManager.add_user_message()  ←── 含图像的消息由 ImageHandler 处理并调用 StateStore 持久化
  │
  ▼
ProviderRouter.stream(messages, tools)
  ...
  └── tool_call_end
      ...
      ToolCallParser.parse()
        │
        ▼
      SafetyGuard.check_tool_call()     ← 安全黑白名单前置检查
        │
        ▼
      HITLController.check()            ← [V2新增] 工具是否包含 requires_approval？
        │                               是：挂起 Task，抛出 ApprovalRequest 给前端，释放锁。
        │                                    （等待唤醒后再继续往下）
        ▼
      SecretManager.inject_secrets()    ← [V2新增] 针对特定工具拦截并动态注入真实凭据 (API Keys)
        │
        ▼
...
# 支持 SQLite 驱动和 Redis 分布式驱动。并在 Provider 产生新 token/message 落地时调用。
```
...
datasets = "^2.18"  # HuggingFace Eval 数据集加载
pandas = "^2.0"     # 结果输出汇聚
...
- [ ] **[V2] StateStore 持久化接口**（集成 SQLite/Redis 实现对话原样加载与追加）
